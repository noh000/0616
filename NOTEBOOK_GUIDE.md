...
| **§4 벡터 스토어** | ChromaDB 컬렉션 + 시드 문서 7건 + `vector_search()` | (RAG 기반) |
...
- RAG: 단순 fan-out (rerank/grading 미구현, 시드 문서 7건)
...
| 파일 | 역할 |
|------|------|
| `manufacturing_agent.ipynb` | 메인 구현 노트북 |
| `build_notebook.py` | 노트북 생성기(이 노트북의 소스 오브 트루스, 수정 후 재생성용) |
